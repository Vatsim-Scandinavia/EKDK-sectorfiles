...
- Lists.txt (EuroScope default list definitions for the Sector Inbound List, Sector Exit List and Departure List)
...
The definitions contain some items from the "Ground Radar plugin", but if that plugin is not in use, the items will only display "???" and can be safely removed. List items only relevant for arrival management and oceanic use are set as not displayed in the list definitions.